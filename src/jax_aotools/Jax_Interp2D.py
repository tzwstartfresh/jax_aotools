...
class jax_interp2d():
    ...
    def get_interp_coff(xaxis, yaxis, signal, cvt_ary, gradient1d, uniform=True):
        ...
        data_ary = jnp.stack([
            ...
            grad_dx[:-1, :-1] * delta_xary, grad_dx[:-1, 1:] * delta_xary, grad_dx[1:, :-1] * delta_xary, grad_dx[1:, 1:] * delta_xary,
    ...
    def interp_func(xi, yi, xaxis, yaxis, coff_ary):
        ...
        xindex, yindex = jnp.clip(jnp.searchsorted(xaxis, xi) - 1, 0, xaxis.shape[-1] - 1), jnp.clip(jnp.searchsorted(yaxis, yi) - 1, 0, yaxis.shape[0] - 1)
